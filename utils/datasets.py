...
NUM_THREADS = min(100, max(1, os.cpu_count() - 1))  # number of multiprocessing threads
global classes, filter_size
...
def create_dataloader(path, imgsz, min_size, batch_size, stride, class_names=[], single_cls=False, hyp=None, augment=False, cache=False, pad=0.0,
...
def xml_to_txt(xml_path):
    ...
    for ob in rot.getElementsByTagName("object"):
        ...
        if cls in classes:
            ...
            rect = [label, c_x/width, c_y/height, w/width, h/height]
        else:
            print(xml_path, cls, '----------- cls not in classes -----------')
            # dst_xml = xml_path.replace('/1/','/2/')
            # shutil.move(xml_path,dst_xml)
            # src_img = xml_path.replace('/labels/','/images/').replace('.xml','.jpg')
            # shutil.move(src_img,src_img.replace('/1/','/2/'))
            # exit()
...
def imgfilter_img(img,path):
    xml_path = path.replace('/images/','/labels/').replace('.jpg','.xml').replace('.png','.xml').replace('.JPG','.xml').replace('.jpeg','.xml').replace('.bmp','.xml')
    if os.path.exists(xml_path) == False:
        print(path)
    label = xml_to_txt(xml_path)
    ...
    for l in label:
        ...
        w = l[3]*width
        h = l[4]*hight
        if w < filter_size or h < filter_size:
            cv2.rectangle(img, (int(x-w/2), int(y-h/2)), (int(x+w/2), int(y+h/2)), (127, 127, 127), thickness=-1)
            # cv2.imwrite(path.split('/')[-1],img)
...
def imgfilter_label(im,l,path):
    ...
    for i in range(len(l)):
        w = l[i][3]*width
        h = l[i][4]*hight
        if w > filter_size and h > filter_size:
            ll.append(l[i])
...
def verify_image_label(args):
    ...
    try:
        ...
        if os.path.isfile(lb_file):
            nf = 1  # label found
            l = xml_to_txt(lb_file)
            l = imgfilter_label(im,l,im_file)
